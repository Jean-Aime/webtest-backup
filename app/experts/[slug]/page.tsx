...
export default async function ExpertPage({ params }: ExpertPageProps) {
  ...
  const expertInsights = insights.filter(i => i.author.id === expert.id);
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {expert.industries.map((industry) => (
                      <a key={industry} href={`/industries/${industry}`} className="block text-sm text-gray-600 hover:text-primary">
                        → {industry.replace('-', ' ')}
                      </a>
                    ))}
                  </div>
